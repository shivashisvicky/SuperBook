...
class _ChapterMarker {
  ...
  final String heading;
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (adventureMatch != null && _looksLikeStructuralHeading(line)) {
  ...
  bool _looksLikeStructuralHeading(String line) {
    ...
    if (excluded.contains(upper)) return false;

    final letters = value.replaceAll(RegExp(r'[^A-Za-z]'), '');
    if (letters.length < 5) return false;
    final upperLetters = letters.replaceAll(RegExp(r'[A-Z]'), '');
    return upperLetters.length / letters.length < 0.35;
  }

  String _cleanStructuralHeading(String value) {
    return value
        .replaceAll(RegExp(r'\\s+'), ' ')
        .replaceAll(RegExp(r'^[|—–:.-]+|[|—–:.-]+
  List<_ChapterMarker> _selectSubstantiveMarkers(
