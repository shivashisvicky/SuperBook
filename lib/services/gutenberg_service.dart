...
class GutenbergService {
  ...
  List<_ChapterMarker> _chapterMarkers(List<String> lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (romanHeadingMatch != null) {
        ...
        final subtitle = romanHeadingMatch.group(2)!.trim().replaceFirst(RegExp(r'[.!?]+
  ...
  bool _looksLikeRomanSectionHeading(String value) {
    ...
    final letters = normalized.replaceAll(RegExp(r'[^A-Za-zÀ-ÖØ-Þ]'), '');
    return letters.length >= 5 && normalized == normalized.toUpperCase();
  }
  ...
  List<_ChapterMarker> _selectSubstantiveMarkers(
    ...
    if (hasTitledRomanSections) {
      // Some scanned editions of works such as The Art of War use headings
      // like "I. LAYING PLANS" rather than "CHAPTER I". Once that form is
      // present, ignore standalone Roman numerals and OCR/reference debris
      // that happen to look numbered.
      return substantive
...
class _ChapterMarker {
  ...
  final bool isTitledRomanHeading;
        ...
        if (_looksLikeRomanSectionHeading(subtitle) && _hasSubstantiveBody(lines, i)) {
          markers.add(_ChapterMarker(
            line: i,
            number: _romanToInt(roman),
            heading: 'Chapter $roman',
            isTitledRomanHeading: true,
          ));
          continue;
        }
      }
