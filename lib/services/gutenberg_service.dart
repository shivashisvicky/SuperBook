...
class _ChapterMarker {
  ...
  final String heading;
    ...
    final adventurePattern = RegExp(r'^(?:ADVENTURE|STORY|PART|BOOK)\s+([IVXLCDM]+|\d+)\.?\s*[-—:.]?\s*(.+)
    for (var i = 0; i < lines.length; i++) {
      final line = lines[i].trim();
      if (line.isEmpty) continue;
      final chapterMatch = chapterPattern.firstMatch(line);
...
, caseSensitive: false);
    final romanHeadingPattern = RegExp(r'^([IVXLCDM]{1,8})\.?\s+(.+)
    for (var i = 0; i < lines.length; i++) {
      final line = lines[i].trim();
      if (line.isEmpty) continue;
      final chapterMatch = chapterPattern.firstMatch(line);
...
).hasMatch(normalized)) return false;
