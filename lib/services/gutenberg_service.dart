...
class GutenbergService {
  ...
  Future<List<GutenbergBookSummary>> _search(String url) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('Gutenberg catalog request failed (' + response.statusCode.toString() + ').');
    }
  ...
  Future<Book> loadBook(GutenbergBookSummary summary) async {
    ...
    final response = await _client.get(Uri.parse('https://gutendex.com/books/' + summary.id.toString()));
  ...
  Book _toBook(GutenbergBookSummary summary, String rawText) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (match != null) markers.add(_ChapterMarker(i, match.group(1)!, match.group(2)!));
    }
    ...
    if (markers.isEmpty) {
      ...
      for (var i = 0; i < markers.length; i++) {
        ...
        final heading = markers[i].heading.isEmpty ? 'Chapter ' + (i + 1).toString() : markers[i].heading;
    ...
    return Book(
      id: 'gutenberg-' + summary.id.toString(), title: summary.title, author: summary.author,
  ...
  Chapter _chapterFromParagraphs(String title, List<String> paragraphs, int index) {
    ...
    return Chapter(
      id: 'chapter-' + (index + 1).toString(), title: title, passage: passage,
...
class _ChapterMarker {
  const _ChapterMarker(this.line, this.number, this.heading);
  final int line;
  final String number;
