      ...
      try {
        const generated = await env.AI.run(IMAGE_MODEL, {
          prompt: [
            ...
            'Sprite source sheet, not a finished scene.',
            'Clean solid chroma-blue background with no texture.',
            'Create an EXACT 4 by 4 grid of equal square cells, with exactly one centered asset per cell and no asset crossing cell boundaries.',
            'Row 1 cells: head, torso, neutral full-body reference, blank.',
            'Row 2 cells: right upper arm, right lower arm, right hand, right foot.',
            'Row 3 cells: left upper arm, left lower arm, left hand, left foot.',
            'Row 4 cells: right upper leg, right lower leg, left upper leg, left lower leg.',
            'Keep identical face, hair, clothing, proportions and illustration style across all parts.',
            'Keep every limb in a neutral anatomical orientation so a local runtime can articulate it around its joint.',
